...
class ParseBrowser(object):
    def __init__(self,
        ...
        self._powershell_script_path = self._parent_dir.joinpath(
            'query_version.ps1'
        )
    ...
    def parse_version(
        ...
        result = subprocess.run(
            ["powershell", "-NoProFile", '-ExecutionPolicy', 'Bypass', '-File', powershell_script_path, '-binary_path', binary_path],
        ...
        for name, installed_path in zip(name_list, installed_path_list):
            ...
            if binary_path.exists():
                browser_version = self.parse_version(
                    powershell_script_path=self._powershell_script_path,
